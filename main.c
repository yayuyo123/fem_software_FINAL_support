...
void read_loading_step(const char *loadingStepFile, StepDisp stepDisp[])
{
    ...
    if(f == NULL)
    {
        printf("error in fopen read_loading_step\nstep-disp file cant open\n");
        return;
    }
    else
    {
        printf("step-disp can open\n");
        //stepデータ読込み
        if(fscanf(f, "step") != 0)
        {
            printf("error in step read_loading_step\n");
            return;
        }
        read_int_csv(f, step);
        //dispデータ読込み
        if(fscanf(f, "disp") != 0)
        {
            printf("error in disp read_loading_step\n");
            return;
        }
...
void get_xboundary(int boundary[], const double mesh[], const ModelSize *rcs)
{
    boundary[0] = 0;
    boundary[1] = JIG;
    boundary[2] = coordinate_to_point((rcs->beam.span - rcs->column.depth) / 2, mesh);
    boundary[3] = coordinate_to_point(rcs->column.center[0], mesh);
    boundary[4] = coordinate_to_point((rcs->beam.span + rcs->column.depth) / 2, mesh);
    boundary[5] = coordinate_to_point(rcs->beam.span, mesh) - JIG;
    boundary[6] = coordinate_to_point(rcs->beam.span, mesh);
}

void get_yboundary(int boundary[], const double mesh[], const ModelSize *rcs, int optf)
{
    boundary[0] = 0;
    boundary[1] = coordinate_to_point((rcs->column.width - rcs->beam.width) / 2, mesh);
    boundary[2] = coordinate_to_point(rcs->beam.center[1], mesh);
    if(optf == 1)
    {
        boundary[3] = coordinate_to_point((rcs->column.width + rcs->beam.width) / 2, mesh);
        boundary[4] = coordinate_to_point(rcs->column.width, mesh);
    }
}

void get_zboundary(int boundary[], const double mesh[], const ModelSize *rcs)
{
    boundary[0] = 0;
    boundary[1] = JIG;
    boundary[2] = coordinate_to_point((rcs->column.span - rcs->beam.depth) / 2, mesh);
    boundary[3] = coordinate_to_point((rcs->column.span + rcs->beam.depth) / 2, mesh);
    boundary[4] = coordinate_to_point(rcs->column.span, mesh) - JIG;
    boundary[5] = coordinate_to_point(rcs->column.span, mesh);
}

Geometry set_geometry(FILE *f, int dir, const ModelSize *rcs, int optf)
{
    Geometry geo;
    geo.mesh = load_mesh(f, dir);
    if(dir == 0)
    {
        get_xboundary(geo.boundary, geo.mesh.length, rcs);
    }
    else if(dir == 1)
    {
        get_yboundary(geo.boundary, geo.mesh.length, rcs, optf);
    }
    else if(dir == 2)
    {
        get_zboundary(geo.boundary, geo.mesh.length, rcs);
    }
    return geo;
}
...
Increment get_column_increment(const Geometry geo[], int optf)
{
    Increment column;
    int bouY = 2;
    if(optf == 1)
    ...
    column.node[2] = column.node[1] * (geo[1].boundary[bouY] - geo[1].boundary[0] + bouY / 2); //ハーフは1、フルは2追加
    column.elm [0] = 1;
    column.elm [1] = geo[0].boundary[4] - geo[0].boundary[2];
    column.elm [2] = column.elm[1] * (geo[1].boundary[bouY] - geo[1].boundary[0]);
    
    return column;
}

Increment get_joint_increment(const Increment *column, const Geometry geo[], int optf)
{
    Increment joint;
    int bouY = 2;
    if(optf == 1)
    ...
    joint.elm[2] = joint.elm[1] * (geo[1].boundary[bouY] - geo[1].boundary[0] + bouY);

    return joint;
}

Increment get_beam_increment(const Geometry geo[], int optf)
{
    Increment beam; 
    int bouY = 2;
    if(optf == 1)
    ...
    beam.node[2] = beam.node[1] * (geo[1].boundary[bouY] - geo[1].boundary[1] + 1);
    beam.elm [0] = 1;
    beam.elm [1] = geo[0].boundary[6];
    beam.elm [2] = beam.elm[1] * (geo[1].boundary[bouY] - geo[1].boundary[1] + 1);
...
void console_boundary(const Geometry geo[], int optf)
{
    int bouY = 2;
    if(optf == 1)
    {
...
void console_increment(const Increment *columnPp, const Increment *jointPp, const Increment *beamPp)
{
    printf("\n------------------[ INCREMENT ]------------------\n");
    printf("         X     Y     Z\n");
    printf("column\n");
    printf("node %5d %5d %5d\n", columnPp->node[0], columnPp->node[1], columnPp->node[2]);
    printf("elm  %5d %5d %5d\n", columnPp->elm[0], columnPp->elm[1], columnPp->elm[2]);
    printf("\njoint\n");
    printf("node %5d %5d %5d\n", jointPp->node[0], jointPp->node[1], jointPp->node[2]);
    printf("elm  %5d %5d %5d\n", jointPp->elm[0], jointPp->elm[1], jointPp->elm[2]);
    printf("\nbeam\n");
    printf("node %5d %5d %5d\n", beamPp->node[0], beamPp->node[1], beamPp->node[2]);
    printf("elm  %5d %5d %5d\n", beamPp->elm[0], beamPp->elm[1], beamPp->elm[2]);
}
...
void console_heads(const column *columnHeads, const joint *jointHeads, const beam *beamHeads)
{
    printf("\n------------------[ HEADS ]------------------\n");
    printf("column\n");
    printf("     :  hexa  fiber   line\n");
    printf("node : %5d  %5d  -----\n", columnHeads->node.hexa, columnHeads->node.fiber);
    printf("elm  : %5d  %5d  %5d\n", columnHeads->elm.hexa, columnHeads->elm.fiber, columnHeads->elm.line);
    printf("\njoint\n");
    printf("     :  quad   film\n");
    printf("node : %5d  -----\n", jointHeads->node.quad);
    printf("elm  : %5d  %5d\n", jointHeads->elm.quad, jointHeads->elm.film);
    printf("\nbeam\n");
    printf("     : number\n");
    printf("node : %5d\n", beamHeads->node);
    printf("elm  : %5d\n", beamHeads->elm);
...
void print_head_template(FILE *f, struct loadNode load, int opt1, int lastStep)
{
    ...
    "UNIT :STRESS=(3) (1:kgf/cm**2  2:tf/m**2  3:N/mm**2=MPa)\n\n", LATEST_UPPDATE, lastStep, node, dir, node, dir);
}

void print_column_axial_force(FILE *f, int startElm, const Geometry geo[], double Fc, const int elmPp[])
{
    int elm;
    double rate = 0.2;
    double unit = rate * Fc;
...
void add_column_hexa(FILE *f, const column *head, const Increment *pp, const Geometry geo[], int optf)
{
    ...
    for(int j = 0; j <= 2; j++)
    {
        ...
        for(int i = 2; i <= 3; i++)
        {
            ...
            if(optf == 1)
            {
                ...
                point.end  [1] = geo[1].boundary[4];
                //節点、要素番号
                index.node += (geo[1].boundary[2] + 1) * pp->node[1];
                index.elm  += geo[1].boundary[2] * pp->elm[1];
                generate_hexa(f, &index, pp, &point, geo, typh[j]);
            }
        }
    }
...
void merge_nodes(FILE *f, int concreteStartNode, const Geometry geo[], const ModelSize rcs, const int pp[])
{
    int node;
    //x直交面
    fprintf(f, "\n----yz\n");
...
void set_pin(FILE *f, int startNode, const Geometry geo[], const int pp[], int opt1)
{
...
void set_roller(FILE *f, int startNode, struct loadNode load, const Geometry geo[], const int pp[], int opt1, int span)
{
...
void cut_surface(FILE *f, const Geometry geo[], int columnStart, int jointStart, int beamStart, const int columnPp[], const int beamPp[])
{
...
void add_typh(FILE *f, const Geometry geo[], const ModelSize rcs, int columnElm, const int columnPp[])
{
    int column;
    int max[2];
    int min[2];
...
column count_column_Numbers(const Geometry geo[], const ModelSize *rcs, int optf)
{
    column numbers;
    int bouY = 2;
    if(optf == 1)
    ...
    numbers.node.hexa  = (geo[0].boundary[4] - geo[0].boundary[2] + 2)
                       * (geo[1].boundary[bouY] - geo[1].boundary[0] + bouY / 2) //ハーフなら1、フルなら2節点追加
    ...
    numbers.elm.hexa = (geo[0].boundary[4] - geo[0].boundary[2])
                     * (geo[1].boundary[bouY] - geo[1].boundary[0])
...
joint count_joint_numbers(const Geometry geo[], const Increment *columnPp, const Increment *jointPp, int optf)
{
    joint numbers;
    int bouY = 2;
    if(optf == 1)
    ...
    //四辺形節点数 - 穴あき部分の節点もカウント - 柱節点と同じインクリメントで数える
    numbers.node.quad  = (geo[0].boundary[4] - geo[0].boundary[2] + 1)    * columnPp->node[0]
                       + (geo[1].boundary[bouY] - geo[1].boundary[0] + 1) * columnPp->node[1]
                       + (geo[2].boundary[3] - geo[2].boundary[2] + 1)    * columnPp->node[2];
    //四辺形要素数 - 接合部要素のインクリメントで計算
    numbers.elm.quad   = (geo[0].boundary[4] - geo[0].boundary[2] + 3)           * jointPp->elm[0]
                       + (geo[1].boundary[bouY] - geo[1].boundary[0] + 2 + optf) * jointPp->elm[1] //ハーフの場合2、フルの場合3追加
                       + (geo[2].boundary[3] - geo[2].boundary[2] + 2)           * jointPp->elm[2];
    //フィルム要素数
    numbers.elm.film   = (geo[0].boundary[4] - geo[0].boundary[2] + 4)       * jointPp->elm[0]
                       + (geo[1].boundary[bouY] - geo[1].boundary[0] + bouY) * jointPp->elm[1] //ハーフの場合2、フルの場合4追加
                       + (geo[2].boundary[3] - geo[2].boundary[2] + 4)       * jointPp->elm[2];

    return numbers;
}

beam count_beam_numbers(const Geometry geo[], int optf)
{
    beam numbers;
    int bouY = 2;
    if(optf == 1)
    ...
    numbers.node = (geo[0].boundary[6] - geo[0].boundary[4] + geo[0].boundary[2])
                 * (geo[1].boundary[bouY] - geo[1].boundary[1] + 1)
...
void out_joint_shear_elment(const Geometry geo[], const column *columnHeads, const joint *jointHeads, const Increment *columnPp, const Increment *jointPp, int optf)
{
    int end = 2;
    if(optf == 1)
    {
    ...
    FILE *f = fopen(OUT_NODE_ELM_FILE, "r");
    if(f == NULL)
    {
        f = fopen(OUT_NODE_ELM_FILE, "w");
        if(f != NULL)
        {
            fprintf(f, "QUAD\n");
            int xOffset = (geo[0].boundary[3] - geo[0].boundary[2]) - 1;
            int zOffset = (geo[2].boundary[3] - geo[2].boundary[2]) / 2;
            for(int i = 0; i <= end; i += 2)
            {
                int start = jointHeads->elm.quad + jointPp->elm[0] + jointPp->elm[1] * (geo[1].boundary[i] + i / 2) + jointPp->elm[2] * zOffset;
                fprintf(f,"QUAD: S(%5d)-E(%5d)-I(%5d) --", start, start + xOffset, jointPp->elm[0]);
                fprintf(f,      "S(%5d)-E(%5d)-I(%5d)\n", start + xOffset + 2, start + 2 * xOffset + 2, jointPp->elm[0]);
            }
            fprintf(f, "HEXA\n");
            xOffset = (geo[0].boundary[4] - geo[0].boundary[2]) - 1;
            zOffset = geo[2].boundary[5] / 2 - 1;
            int start = columnHeads->elm.hexa + columnPp->elm[2] * zOffset;
            for(int i = 0; i <= 1; i++)
            {
                fprintf(f,"HEXA: S(%5d)-E(%5d)-I(%5d)\n", start, start + xOffset, columnPp->elm[0]);
                if(i == 0)
                {
                    fprintf(f, "...INC(%5d)-SET(%5d)\n", columnPp->elm[1], geo[1].boundary[end] - 1);
                }
                start += columnPp->elm[1] * (geo[1].boundary[end] - 1);
            }
            fclose(f);
        }
        else
    ...
    {
        printf(OUT_NODE_ELM_FILE " already exist\n");
        fclose(f);
    }
    return ;
}

void modeling_rcs(const char *inputFileName, int opt1, int optf, const char *stepDispFile)
{
    /*todo
        ---要素タイプ---
        film要素
   ...
   */
    ...
    */
    ...
    const Geometry  modelGeometry[XYZ] = {set_geometry(fin, 0, &rcs, optf), set_geometry(fin, 1, &rcs, optf), set_geometry(fin, 2, &rcs, optf)};
    fclose(fin);
    ...
    //境界線情報表示
    console_boundary(modelGeometry, optf);

    //インクリメント
    const Increment columnPp = get_column_increment(modelGeometry, optf);
    const Increment jointPp  = get_joint_increment(&columnPp, modelGeometry, optf);
    const Increment beamPp   = get_beam_increment(modelGeometry, optf);
    console_increment(&columnPp, &jointPp, &beamPp);

    //各部分の必要番号数
    const column columnNumbers = count_column_Numbers(modelGeometry, &rcs, optf);
    const joint  jointNumbers  = count_joint_numbers(modelGeometry, &columnPp, &jointPp, optf);
    const beam   beamNumbers   = count_beam_numbers(modelGeometry, optf);
    ...
    console_heads(&columnHeads, &jointHeads, &beamHeads);

    out_joint_shear_elment(modelGeometry, &columnHeads, &jointHeads, &columnPp, &jointPp, optf);
    ...
    */
    ...
    //柱六面体要素
    add_column_hexa(fout, &columnHeads, &columnPp, modelGeometry, optf);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    if(inputFile != NULL){
        ...
        modeling_rcs(inputFile, optb, optf, stepDispFile);
    }
>>>>>>> bd9a22c71def681f1bcf1dc23639e8e88a38a427
    return 0;
}